...
namespace Arbitrary_NTT_Solver
{
    ...
    struct MOD3
    {
        ...
        void init(int val)
        {
            reminder[0] = reminder[1] = reminder[2] = val;
        }
        MOD3 inv() const
        {
            MOD3 ret;
...
struct BigNumber
{
    ...
    BigNumber trans(int out_radix) const
    {
        ...
        // int *stack_num = new int[k + 1];
        BigNumber stack_bignum[k + 1];
        int stack_num[k + 1];
        int top = 0;
        ...
        ret.f = f;
        // delete []stack_bignum, delete[]f_gap, delete stack_num;
        return ret;
    }
    ...
    void output() const
    {
        ...
        for (int i = is_neg; i < print_len + is_neg - 1 - i; i++)
            ch[i] ^= ch[print_len + is_neg - 1 - i], ch[print_len + is_neg - 1 - i] ^= ch[i],
                ch[i] ^= ch[print_len + is_neg - 1 - i];
        ...
        for (int i = len - 2; i >= 0; i--)
        {
            ...
            while (k >= 0)
                ch[print_len + k] = num_to_char[0], k--;

            print_len += MODD;
        }

        if (print_len == 0)
            ch[print_len++] = num_to_char[0];
...
// below is test area and sample for Base Conversion from A(in_radix) to B(out_radix)
struct fastIO
{
...
int main()
{
    scanf("%d%d%s", &in_radix, &out_radix, s);
    a = BigNumber(s, in_radix);
    b = a.trans(out_radix);
    b.output();
}
